...
#main function
{
	...
	if(/^ATOM  /){
	    ...
	    $res = $atm{res_num};
	    if(!$first_pass){
		if (abs($res-$last_res)>1){ 
                  $non_seq_flag = 1;
                  print "err:\t$res\t$last_res\n";
                }
	    }
    ...
    print "Warning: nonsequential numbering in PDB!\n" if $non_seq_flag;
...
#   print "$atom{atom_num}\t$atom{atom_name}\t$atom{res_type}\t$atom{res_num}\t$atom{x}\t$atom{y}\t$atom{z}\t$atom{chain_id}\n";
    return %atom;
}
